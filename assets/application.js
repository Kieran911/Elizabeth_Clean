...
async function renderCartDrawer() {
  ...
  if (
    updatedCart &&
    updatedCart.sections &&
    updatedCart.sections['cart-drawer']
  ) {
...
function attachCartDrawerEventListeners() {
  ...
  const quantityUpdateButtons = cartSidebar.querySelectorAll(
    'button[name="update"]'
  );
  ...
  if (form) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.description || 'Failed to update cart quantity'
        );
      }
...
async function handleRemoveItemButton(event) {
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json();
      throw new Error(
        errorData.description || 'Failed to remove item from cart'
      );
    }
    ...
    form.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.description || 'Failed to add item to cart'
          );
        }
